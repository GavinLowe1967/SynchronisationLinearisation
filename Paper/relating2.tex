...
operation of the synchronisation specification object to the parameters $\sm
x_1, \ldots, \sm x_k$ to obtain the results $\sm y_1, \ldots, \sm y_k$; it
stores the first $k-1$ in appropriate |returns|\s{i} arrays, and returns $\sm
y_k$ itself.  In the case $k=4$, it has definition:
%
\begin{scala}
...
of the specification object.  The specification object is in Figure~\ref{fig:TwoStepHomoLinSpec}, and encodes
...
\end{scala}
\caption{A two-step synchronisation specification object for homogeneous
  synchronisations.}
\label{fig:TwoStepHomoLinSpec}
\end{figure}
