...
%Figure~\ref{fig:two-step-closeable-chan} 
...
specification object performs operations  as follows:
(1)~$\op_{i_i}(x_1), \ldots, \op_{i_a}(x_a)$, in that order; (2) some
$\overline\op_{i_i}$, which calls \sync\ on the synchronisation linearisation
object and stores the results for other threads; (3)~the remaining
$\overline\op_{i_j}$, in some order, which retrieve their results.
