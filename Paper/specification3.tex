...
We now consider a liveness condition for synchronisation objects, which builds
on synchronisation linearisability.  
...
return event~$r$ is anticipated after~$h$, but $h$ can lead to a state where
$r$ is refused.

We say that a history is \emph{synchronisation-progressible} if, for very
...
$\return.\receive^3\::5$ cannot be refused (corresponding to~$h_s^2$).  One of
the synchronisations should happen, and then the relevant operations should be
able to return in some order.
