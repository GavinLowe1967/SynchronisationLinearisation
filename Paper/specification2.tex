...
%%  via extension $h_e = h
%% \cat \seq{ \return.\send^1 \:: () }$ (and no other histories).  The channel
%% may not block after~$h$ since there is a $\return$ event in~$h_e$ that is not
%% in~$h$.  Informally, the send should be able to return, since it has
%% synchronised. 
\end{example}
...
%% synch\-ronisation-linearisable in two ways:
%% \begin{itemize}
%% \item With respect to $h_s = \linebreak[1] \seq{\sync^{1,2}(3,()) \:: ((),3)}$
%%   via extension $h_e = h \cat \seq{\return.\send^1\::(),\linebreak[1]
%%     \return.\receive^2\::3}$ (or with the $\return$ events in the opposite
%%   order); but there are $\return$ events in~$h_e$ but not~$h$.
  
%% \item With respect to $h_s = \seq{}$ via extension $h_e = h$; but it is
%%   also synchronisation-linearisable with respect to the extensions $h_s =
%%   \linebreak[1] \seq{\sync^{1,2}(3,()) \:: ((),3)}$ and $h_e = h
%%   \cat \seq{\return.\send^1\::(),\linebreak[1] \return.\receive^2\::3}$.
%% \end{itemize}
%% %
%% Thus the channel may not block after~$h$, since neither case satisfies the
%% conditions.  Informally, the two invocations should be able to synchronise (if
%% they haven't already), and then both should be able to return.
...
%% *** That's not quite right.  We want to say that all relevant returns are
%% possible, at least in the CSP models.  I don't think we want to make that
%% distinction here.

%% \begin{definition}
%% Let $Sync$ be a synchronisation object that is synchronisation-linearisable
%% with respect to specification object $Spec$.  Let $h$ be a (not necessarily
%% complete) history of~$Sync$. We say that $h$ is \emph{maximally
%%   synchronisation-linearisable} if there is a legal history $h_s$ of~$Spec$
%% such that
%% %
%% \begin{itemize}
%% \item $complete(h)$ and $h_s$ are synchronisation linearisable;

%% \item There are no proper extension~$h'$ of~$h$ and proper extension~$h_s'$
%%   of~$h_s$ such that $complete(h')$ and~$h_s'$ are synchronisation compatible.
%% \end{itemize}
%% \end{definition}

%% \begin{lemma}
%% Any synchronisation-linearisable history can be extended to a maximally
%% synchronisation-linearisable history.  
%% \end{lemma}

%% \begin{definition}
%% Let $h$ be a synchronisation-linearisable history, and let $h_m$ be an
%% extension that is maximally synchronisation-linearisable.  Then we say that
%% the pending $\return$ events are those in~$h_m$ that are not in~$h$.

%% Consider an execution~$e$ of~$Sync$.  We say that a return event~$ret$ is
%% \emph{eventually available} if in every extension~$e'$ of~$e$ with no other
%% $\call$ or $\return$ event, there is an extension~$e''$ of~$e'$ where the next
%% event is~$ret$. 

%% Given a history~$h$ of~$Sync$, we say that a .............
%% \end{definition}
