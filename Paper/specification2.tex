...
specification, via a synchronisation specification object; and defining which
operations of the concrete synchronisation object can synchronise together,
and relating them to the corresponding operation of the synchronisation
...
class CombiningBarrierSpec{
  def sync((id£\s1£,x£\s1£): (Int,A) ..., (id£\s{\ss n}£,x£\s{\ss n}£): (Int,A)) = {
...
and of the synchronisation specification object via a \emph{synchronisation
  relation}: this relates a sequence of operations of the synchronisation
object to the corresponding operation of the synchronisation specification
object.  For the combining barrier, the relation is
\[
\begin{align}
\set{ ( 
  \seq{ \sm{sync}((id_1,x_1)), \ldots, \sm{sync}((id_{\ss n},x_{\ss n})) },\;
  \sm{sync}((id_1,x_1), \ldots, (id_{\ss n},x_{\ss n})) 
) \;\mid\null \\
...
corresponding specification object is:
%
\begin{scala}
...
synchronising $\sm{sync}_r$ corresponds to a receive returning
without synchronising, and $\sm{sync}_{s,r}$ corresponds to a send and receive
synchronising, as captured by the synchronisation relation:
\[
\begin{align}
\set{ ( \seq{ \sm{send}(x) },\, \sm{sync}_s(x) ) \midd x \in \sm A } \union{} 
\set{ ( \seq{ \sm{receive}(()) },\, \sm{sync}_r(()) ) } \union{}  \\
\set{ ( \seq{ \sm{send}(x), \sm{receive}(()) },\, \sm{sync}_{s,r}(x,()) ) \midd
      x \in \sm A }.
\end{align}
\]
...
corresponding to |sendFail| and |receiveFail|.  The synchronisation relation is
\[
\begin{align}
\set{ ( \seq{\sm{close}(())}, \sm{close}(()) ) } \union
\set{ ( \seq{ \sm{send}(x) },\, \sm{sendFail}(x) ) 
  \midd x \in \sm A } \union{} \\
\set{ ( \seq{ \sm{receive}(()) },\, \sm{receiveFail}(()) ) } \union 
\set{ ( \seq{ \sm{send}(x), \sm{receive}(()) },\, \sm{sync}(x,()) ) \midd
      x \in \sm A }.
\end{align}
\]
