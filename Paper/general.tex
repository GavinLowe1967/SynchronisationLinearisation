...
\end{enumerate}
%
No other operations will be linearised between the start of stage~1 until
after stage~2; but other operations may be linearised during stage~3.

During stage~1, the two-step linearisabilty tester will record the prefix of
...
Figure~\ref{fig:schema}.  The variable~|state| stores the sequence of |op|$_i$
linearised so far: this satisfies an invariant that it is a prefix of an
element of $\dom \A$.  For each thread~|t|,\, |returns(t)| stores the value to
be returned by~|t| as part of its most recent uncompleted synchronisation, if
any.
...
\begin{scala}
trait Op
case class Op_i(t: ThreadID, x: A_i) //  for £$i = 1,...,n$.£
type State = List[Op]
