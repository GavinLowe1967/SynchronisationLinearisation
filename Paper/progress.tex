\section{Specifying liveness}
\label{sec:progress}

We now consider a liveness condition for synchronisation objects.  Fix a
synchronisation object |SyncObj| and a synchronisation specification object
|SyncSpec|.  We assume that |SyncObj| is synchronisation-linearisable with
respect to~|SyncSpec|.
...
We say that a system is \emph{synchronisation-progressible} (with respect to
the synchronisation specification object) if, for every history~$h$, it cannot
fail to progress after~$h$.
\end{definition}
...
In fact, any synchronisation object without unary synchronisations is
necessarily not lock-free: if a particular operation is unable to synchronise,
...
and only if each object $o \in \O$ is synchronisation-progressible.
\end{prop}
