\section{Direct testing of synchronisation linearisation and progressibility}
\label{sec:direct}
...
a failure of progressibility; but if there were a second execution
of~$\op_2$, it would unblock both executions of~$\op_1$, so all executions
would return, and the failure of progressibility would be missed.
...
positives.  We discuss this point further in Section~\ref{sec:experiments}. 
...
it is synchronisation progressible.  These algorithms again assume that the
...
object, the problem of deciding whether a history is
synchronisation-linearisable is NP-complete in general: this is not
surprising, given a similar result for standard linearisation.

Things are more interesting in the stateless case.  (In practice,
linearisation is only ever considered in the stateful case; for a stateless
object, deciding linearisability amounts to checking each operation execution
in isolation, which can be decided in linear time.)  We show that in the case
of \emph{binary} synchronisations with a \emph{stateless} specification
object, synchronisation linearisability can be decided in polynomial time: we
consider the heterogeneous case in Section~\ref{sec:binary-heterogeneous}, and
the homogeneous case in Section~\ref{sec:binary-homogeneous}.  Nevertheless,
in Section~\ref{sec:non-binary-stateless} we show that for synchronisations of
three or more executions, the problem is again NP-complete, even in the
stateless case.

The table below summarises the complexity results for deciding synchronisation
linearisation for a history of length~$n$.
% 
\begin{center}
\begin{tabular}{cc}
...
Let |ConcVariable| be a concurrent datatype that represents an integer variable.
