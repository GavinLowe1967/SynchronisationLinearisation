...
class DeadlockOneFamily(n: Int) extends OneFamilyT{
  /** synced(i)(j) is true if threads i and j have synchronised. */
  private val synced = Array.ofDim[Boolean](n,n)
  ...
  def sync(me: Int): Int = {
    require(0 <= me && me < n)
    require((0 until n).exists(him => him != me && !synced(me)(him)))
    /* This attempts to do short-term locking for reading of waiting(him). */
    def getWaiting(him: Int) = {
      mutex.down; val res = waiting(him); mutex.up; res 
    }
    var him = 0
    while(him < n && (him == me || synced(me)(him) || !getWaiting(him)))
      him += 1
    mutex.down
    if(him < n){ // sync with him
      lastSync = me; synced(me)(him) = true
  ...
  /* This goes wrong because two threads could concurrently search through
   * waiting, and miss that they could synchronise with one another. */
}
